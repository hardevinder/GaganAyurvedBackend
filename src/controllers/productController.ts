...
/* ---------------------------
   ...
   --------------------------- */
function findCycle(root: any, maxDepth = 12, maxNodes = 20000): string | null {
  const seen = new WeakSet();
  const stack: Array<{ key: string; node: any }> = [];
  let nodesVisited = 0;

  function dfs(node: any, path: string, depth: number): string | null {
    if (nodesVisited++ > maxNodes) return `TRUNCATED (visited>${maxNodes})`;
...
async function ensureUploadDir() {
  ...
  if (!existsSync(dirToUse)) mkdirSync(dirToUse, { recursive: true });
  await fs.access(dirToUse).catch(() => fs.mkdir(dirToUse, { recursive: true }));
}

function buildLocalUrl(relPath: string) {
  if (!PUBLIC_BASE_URL) return relPath;
  const p = relPath.startsWith("/") ? relPath : "/" + relPath;
  return PUBLIC_BASE_URL.replace(/\/$/, "") + p;
}

async function deleteLocalFile(relPath?: string) {
  if (!relPath) return;
  try {
    const filename = path.basename(relPath);
    const filepath = path.isAbsolute(relPath) ? relPath : path.join(UPLOAD_DIR, filename);
...
function serializeProductSafe(raw: any) {
  if (!raw) return raw;
  const prod: any = {
    id: raw.id,
    name: raw.name,
...
/* ---------------------------
   ...
   --------------------------- */
async function collectFilesFromRequest(request: FastifyRequest) {
    ...
    for (const f of imgs) {
      try {
        ...
        if (f && (f.filepath || f.path)) {
          ...
          const pushed = {
            originalname: f.filename || f.originalname || path.basename(filepath),
            path: filepath,
        ...
        if (!buffer) {
          if (f && Buffer.isBuffer(f.buffer)) buffer = f.buffer;
          else if (f && Buffer.isBuffer(f.file)) buffer = f.file;
          else if (f && Buffer.isBuffer(f._buf)) buffer = f._buf;
          else if (f && Array.isArray(f._buf) && f._buf.every((n: any) => typeof n === "number")) {
            try {
        ...
        const pushedFallback = {
          originalname: f && (f.filename || f.originalname) ? (f.filename || f.originalname) : path.basename(String(f?.url || "file")),
...
/* ---------------------------
   ...
   --------------------------- */
function extractFormValue(raw: any): string | undefined {
...
/* ---------------------------
   ...
   --------------------------- */
function normalizeIncomingImage(i: any, index = 0) {
  ...
  if ((!url || url === "/" || url === folderPrefix) && alt) {
    const filenameFromAlt = path.basename(alt);
...
/* ---------------------------
   ...
   --------------------------- */
function buildValidatedVariants(arr: any[]): any[] {
  const toNumberOrUndefined = (x: any) => {
...
export const createProduct = async (request: FastifyRequest, reply: FastifyReply) => {
  ...
  const {
    ...
    metaTitle: bodyMetaTitle,
    metaDesc: bodyMetaDesc,
  } = bodyAny;

  // Accept variants field from either request.body or request.fields
  const variantsField = bodyVariants ?? (request as any).fields?.variants;
  ...
  const rawName = (bodyAny && bodyAny.name) ?? (request as any).fields?.name;
  const rawSlug = (bodyAny && bodyAny.slug) ?? (request as any).fields?.slug;
  const rawDescription = (bodyAny && bodyAny.description) ?? (request as any).fields?.description;
  ...
  const imagesCreateRaw = files.map((f: any, idx: number) => {
    const filename = f.path ? path.basename(f.path) : "";
  ...
  let existingImagesFromClient: any[] | undefined;
  try {
    const raw = bodyAny.existingImages ?? (request as any).fields?.existingImages;
    if (raw) {
      const arr = typeof raw === "string" ? JSON.parse(raw) : raw;
      if (Array.isArray(arr)) existingImagesFromClient = arr.map((it: any, idx: number) => normalizeIncomingImage(it, idx));
    }
  } catch (err) {
  ...
  let variantCreates: any[] | undefined;
  if (variants) {
    try {
  ...
  try {
    ...
    if (STORAGE_DRIVER === "local" && PUBLIC_BASE_URL && resp.images) {
      resp.images = resp.images.map((img: any) => ({ ...img, url: buildLocalUrl(img.url) }));
    }
    ...
    if (err?.code === "P2002") return reply.code(409).send({ error: "Unique constraint failed", meta: err.meta });
...
export const updateProduct = async (request: FastifyRequest, reply: FastifyReply) => {
  ...
  if (Number.isNaN(id)) return reply.code(400).send({ error: "Invalid product id" });
  ...
  const rawName = bodyName ?? (request as any).fields?.name;
  const rawSlug = bodySlug ?? (request as any).fields?.slug;
  const rawDescription = bodyDescription ?? (request as any).fields?.description;
  ...
  // variants parse + sanitize (derive price from price || salePrice || mrp)
  let variantReplaceCreates: any[] | undefined;
  if (variantsField) {
    try {
  ...
  if (removeImageIds) {
    try {
      ...
      if (Array.isArray(arr)) removeIds = arr.map((x) => Number(x)).filter((n) => !Number.isNaN(n));
    } catch {
      // ignore
    }
  }
  ...
  try {
    const updated = await prisma.$transaction(async (tx) => {
      ...
      if (!existing) throw Object.assign(new Error("NotFound"), { code: "P2025" });
    ...
    if (updated.deletedImages && updated.deletedImages.length) {
      await Promise.all(
        updated.deletedImages.map(async (img: any) => {
          ...
          if (STORAGE_DRIVER === "s3") {
            try {
              ...
              if (!key) {
                ...
                if (idx !== -1) key = url.slice(idx + ".amazonaws.com/".length);
              }
    ...
    if (err?.code === "P2002") return reply.code(409).send({ error: "Unique constraint failed", meta: err.meta });
    if (err?.code === "P2025" || err?.message === "NotFound") return reply.code(404).send({ error: "Product not found" });
    if (err?.code === "CIRCULAR_PAYLOAD") return reply.code(400).send({ error: "Invalid payload (circular reference in data)", detail: err.detail || null });
    return reply.code(500).send({ error: err?.message || "Internal error" });
  }
};
...
export const deleteProduct = async (request: FastifyRequest, reply: FastifyReply) => {
  ...
  try {
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      if (!product) throw Object.assign(new Error("NotFound"), { code: "P2025" });
